...
//карта с маркерами + переключение
...
//список маркеров и id где оставлены отзывы
void _openDrawer() {
  ...
  final reviewedMarkersList = [
    ...
    ..._additionalMarkers.where((marker) => reviewedMarkers.contains(marker.markerId.value)),  //Проверяет все маркеры
  ];
...
//подтверждение удаления добавленной фотки или отмена
...
void _deleteReview(String markerId) {
  setState(() {
    _reviews.removeWhere((review) => review['markerId'] == markerId); 
  });
}
